...
const MovieListFetcher = () => {
    ...
    const [sortOrder, setSortOrder] = useState("asc"); // Добавлено состояние для порядка сортировки
    ...
    useEffect(() => {
        let sortedMovies = [...allMovies]; // Создаем копию массива всех фильмов

        // Сортировка по году
        if (filterType === "year") {
            sortedMovies.sort((a, b) => {
                return sortOrder === "asc" ? parseInt(a.year, 10) - parseInt(b.year, 10) : parseInt(b.year, 10) - parseInt(a.year, 10);
            });
        }
        ...
        // Обновляем список отсортированных фильмов
